...
import { Tool } from '../lib/toolsService';
...
function mapRowToTool(row: CsvRow): Tool {
  return {
    name: row.name || row.tool || 'Unnamed Tool',
    category: (row.category as Tool['category']) ?? 'metin',
    ...
    pricing: {
      ...
      currency: (row.currency as Tool['pricing']['currency']) || 'USD',
    },
